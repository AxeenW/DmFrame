namespace DmFrame.Classes
{
    public static class Defaults
    {
        public static class Sized
        {
            public static readonly int DesktopWidth = 800;          // 主視窗寬度
            public static readonly int DesktopHeight = 600;         // 主視窗高度
            public static readonly int BoarderSize = 2;             // 邊框大小

            public static readonly int DesktopMenuMaxWidth = 168;   // 主視窗選單最大寬度
            public static readonly int DesktopMenuMinWidth = 48;    // 主視窗選單最小寬度
            public static readonly int LeftRightWidth = 8;
        }
    }
}
